...
declare_id!("22oc9tz17ZY35UX1W5YFyr9WMN2T4ueYN2a5jpeKGUC9");
...
pub mod transfer_hook {
    ...
    pub fn initialize_extra_account_meta_list(
        ...
        authority_pda.authority = ctx.accounts.payer.key();

        Ok(())
    }
     
     pub fn withdraw_delegate_funds(ctx: Context<WithdrawDelegateFunds>, amount: u64) -> Result<()> {

    ctx.accounts.verify_authority()?;

    let delegate_balance = ctx.accounts.delegate_wsol_token_account.amount;

    if amount > delegate_balance {
        return err!(ErrorCode::InsufficientFunds);
    }

    let signer_seeds: &[&[&[u8]]] = &[&[b"delegate", &[ctx.bumps.delegate]]];

    // Transfer wSOL from delegate ATA to the recipient's wSOL account
    transfer_checked(
        CpiContext::new_with_signer(
            ...
            TransferChecked {
                ...
                to: ctx.accounts.recipient_wsol_token_account.to_account_info(),
...
pub struct InitializeExtraAccountMetaList<'info> {
    ...
    #[account(init, seeds = [b"authority"], bump, payer = payer, space = 8 + 32)]
...
#[derive(Accounts)]
pub struct WithdrawDelegateFunds<'info> {
    #[account(mut)]
    ...
    pub recipient_wsol_token_account: InterfaceAccount<'info, TokenAccount>,
    pub wsol_mint: InterfaceAccount<'info, Mint>,
    pub token_program: Program<'info, Token>,
...
impl<'info> WithdrawDelegateFunds<'info> {
    pub fn verify_authority(&self) -> Result<()> {
        let authority_pda_account = &self.authority_pda;
        if authority_pda_account.authority != self.recipient.key() {
            msg!("Unauthorized access: recipient is not the authority");
...
pub enum ErrorCode {
    // ... other errors ...
    #[msg("Insufficient funds in delegate account")]
    InsufficientFunds,
